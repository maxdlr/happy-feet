...
DuAEItem.compareItems = function (itemA, itemB) {
...
DuAEItem.usedIn = function (item, recursive, keepExpressionOnly) {
  ...
  if (
    currentTime - DuAEProject.lastExpressionCacheUpdateTime > 60000 ||
    DuAEProject.expressionCache.length == 0
  ) {
  ...
  if (item instanceof CompItem)
    regExp = new RegExp("comp\\s*\\(\\s*(\"|')" + itemNameRegEx + "\\1\\s*\\)");
  else
    regExp = new RegExp(
      "footage\\s*\\(\\s*(\"|')" + itemNameRegEx + "\\1\\s*\\)",
    );
  ...
  if (recursive) {
    for (var i = 0, num = usedIn.length; i < num; i++) {
      usedIn = usedIn.concat(
        DuAEItem.usedIn(usedIn[i], true, keepExpressionOnly),
      );
    }
  }
...
DuAEItem.isInFolder = function (item, folder) {
  ...
  if (pFolder.id == app.project.rootFolder.id && pFolder.id == folder.id)
    return true;
